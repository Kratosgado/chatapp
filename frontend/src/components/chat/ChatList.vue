...
function getAvatar(chat: ChatRoom) {
    return chat.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.name)}&background=random`
}
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto">
      <div v-if="chatsStore.loading && chats.length === 0" class="p-4 space-y-4">
      ...
      <div
        ...
        :class="[
          ...
          currentChat?.id === chat.id && 'bg-primary-50 dark:bg-primary-900/10 border-l-4 border-l-primary-500',
        ]"
      >
        <div class="flex items-center gap-3">
          <UAvatar :src="getAvatar(chat)" :alt="chat.name" />
          <div class="flex-1 min-w-0">
            <h3 class="font-semibold text-gray-900 dark:text-white truncate">{{ chat.name }}</h3>
          ...
          <div class="flex flex-col items-end">
             <span class="text-xs text-gray-400" v-if="chat.lastMessage">{{ formatTime(chat.lastMessage.sentAt) }}</span>
          </div>
        </div>
      </div>
       <div v-if="filteredChats.length === 0 && !chatsStore.loading" class="p-8 text-center text-gray-500">
          No chats found
      </div>
    </div>
