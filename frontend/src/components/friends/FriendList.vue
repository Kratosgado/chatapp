...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- Requests -->
    <div v-if="requests.length > 0" class="p-4 border-b border-gray-200 dark:border-gray-800">
      <h3 class="text-sm font-semibold text-gray-500 mb-2">Requests</h3>
      <div class="space-y-2">
        <div v-for="req in requests" :key="req.id" class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <UAvatar :src="getAvatar(req.sender)" :alt="req.sender.name" size="xs" />
            <span class="text-sm font-medium">{{ req.sender.name }}</span>
          </div>
          <div class="flex gap-1">
            <UButton icon="i-heroicons-check" size="xs" color="green" variant="ghost" @click="handleAccept(req.id)" />
            <UButton icon="i-heroicons-x-mark" size="xs" color="red" variant="ghost" @click="handleReject(req.id)" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-4">
      <div v-if="friends.length === 0" class="text-center text-gray-500 text-sm mt-4">
