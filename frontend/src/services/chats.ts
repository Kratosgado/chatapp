...
export const createChat = async (participants: string[], name?: string): Promise<ChatRoom> => {
  const response = await api.post('/chats', { participants, name })
  return response.data
}

export const getMessages = async (chatId: string, page = 0, size = 20): Promise<Message[]> => {
