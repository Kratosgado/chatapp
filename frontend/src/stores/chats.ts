...
import { getChats, createChat, getMessages, sendMessage } from '@/services/chats'
import type { ChatRoom, Message } from '@/types'
import { connect, subscribe, disconnect, sendMessage as wsSendMessage } from '@/services/websocket'
...
export const useChatsStore = defineStore('chats', () => {
  ...
  const create = async (participants: string[], name?: string) => {
  ...
  const setCurrentChat = (chat: ChatRoom) => {
    ...
    chatSubscription = subscribe(`/topic/chats/${chat.id}`, (message: Message) => {
    ...
    // Subscribe to typing
    typingSubscription = subscribe(`/topic/chats/${chat.id}/typing`, (payload: { userId: string, isTyping: boolean }) => {
